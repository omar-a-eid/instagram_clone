    ...
    <aside class="lg:col-span-5 h-full scrollbar-hide relative flex gap-4 flex-col overflow-hidden overflow-y-scroll">
        ...
        <footer class="mt-auto sticky border-t bottom-0 z-10 bg-white">
            ...
            @if ($post->totalLikers > 0 && !$post->hide_like_view)
                <p class="font-bold text-sm">{{ $post->totalLikers }} {{ $post->totalLikers > 1 ? 'Likes' : 'Like' }}
                </p>
            @endif
            {{-- name and comment --}}
                ...
                <p> <strong class="font-bold">{{ $post->user->name }}</strong>
                    @php
                        ...
                        foreach ($words as $word) {
                            if (strpos($word, '#') === 0) {
                                $url = strtolower(substr($word, 1));
                                $txt .=
                                    ' ' .
                                    '<a  class="text-blue-700" href= ' .
                                    route('tag.show', ['name' => $url]) .
                                    '>' .
                                    $word .
                                    '</a>';
                            } else {
            ...
            @if ($post->allow_commenting)
                {{-- view post modal --}}
                <button class="text-slate-500/90 text-sm font-medium"> Total {{ $post->comments->count() }}
                    comments</button>
                {{-- addComment --}}
